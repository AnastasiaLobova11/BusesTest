...
class BusesTest5 {
    ...
    //тест проверяет что при создании нескольких автобусов, нумерация маршрутов для каждого из них начинается с 1
    @Test
    void twoBuses() throws Exception {
        bs = new Buses();
    ...
    @Test
    void twoSimilarStops() throws Exception {
    ...
    //если возникает при добавлении какая-то иная ошибка, то говорим о том, что произошла непредвиденное исключение
    @Test
    void addBus() {
        bs = new Buses();
        int result;
    ...
    public void testSbusesForStop() throws Exception {
        ...
        Assertions.assertTrue(Arrays.equals(bsL, result.toArray()));

    }

    //SbusesForStop, рассматриваем случай когда есть 1 маршрут, и он проходит через указанную остановку
    @Test
    public void testSbusesForStop1() throws Exception {
        ...
        String bsL1[] = {"85"};
        Assertions.assertTrue(Arrays.equals(bsL1, result1.toArray()));

    }

    //SbusesForStop, рассматривается ситуация, когда через указанную остановку не проходит ни один маршрут
    @Test
    public void testSbusesForStopException() throws Exception {
    ...
    //NbusesForStopException, NbusesForStop рассматриваем случай когда есть несколько маршрутов, и некоторые из них проходят через указанную остановку
    @Test
    ...
    //NbusesForStopExceptionБ рассматриваем случай когда есть 1 маршрут, и он проходит через указанную остановку
    @Test
    ...
    public void testStopsForBus16() throws Exception {
        ...
        for (Map.Entry<String, Set<Integer>> e : result.entrySet()) {
            String s = e.getKey();
            Assertions.assertTrue(Arrays.asList(exStop).contains(s));

            Set<Integer> v = e.getValue();
            Assertions.assertTrue(exNum.contains(v));
        }
    ...
    //StopsForBus, проверяет выбрасываемое исключение с текстом No bus для, когда указанно автобуса нет
    @Test
    ...
    public void testAllBuses() throws Exception {
        ...
        Assertions.assertTrue(Arrays.equals(s, bs.allBuses().toArray()));
